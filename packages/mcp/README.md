...
Aliases such as `sheet` → `drawer` and `gallery` → `slider` work on search, add, diff, and update. `carousel` is the horizontal scroller (`ply-horizontal-carousel`), not the gallery slider.
